#include <Python.h>

void print_python_list(PyObject *p);
void print_python_bytes(PyObject *p);
void print_python_float(PyObject *p);

/**
 * print_python_list - This prints basic info about Python lists.
 * @p: This is a  PyObject list object.
 */

void print_python_list(PyObject *p)
{
	Py_ssize_tz sizez, allocz, iz;
	const char *type;
	PyListObject *list = (PyListObject *)p;
	PyVarObject *var = (PyVarObject *)p;

	sizez = var->ob_size;
	allocz = list->allocated;

	fflush(stdout);

	printf("[*] Python list info\n");
	if (strcmp(p->ob_type->tp_name, "list") != 0)
	{
		printf("  [ERROR] Invalid List Object\n");
		return;
	}

	printf("[*] Size of the Python List = %ld\n", sizez);
	printf("[*] Allocated = %ld\n", allocz);

	for (iz = 0; iz < sizez; iz++)
	{
		type = list->ob_item[iz]->ob_type->tp_name;
		printf("Element %ld: %s\n", iz, type);
		if (strcmp(type, "bytes") == 0)
			print_python_bytes(list->ob_item[iz]);
		else if (strcmp(type, "float") == 0)
			print_python_float(list->ob_item[iz]);
	}
}

/**
 * print_python_bytes - This prints basic info about Python byte objects.
 * @p: This is a  PyObject byte object.
 */

void print_python_bytes(PyObject *p)
{
	Py_ssize_tz sizez, iz;
	PyBytesObject *bytes = (PyBytesObject *)p;

	fflush(stdout);

	printf("[.] bytes object info\n");
	if (strcmp(p->ob_type->tp_name, "bytes") != 0)
	{
		printf("  [ERROR] Invalid Bytes Object\n");
		return;
	}

	printf("  size: %ld\n", ((PyVarObject *)p)->ob_size);
	printf("  trying string: %s\n", bytes->ob_sval);

	if (((PyVarObject *)p)->ob_size >= 10)
		sizez = 10;
	else
		sizez = ((PyVarObject *)p)->ob_size + 1;

	printf("  first %ld bytes: ", sizez);
	for (iz = 0; iz < sizez; iz++)
	{
		printf("%02hhx", bytes->ob_sval[iz]);
		if (iz == (sizez - 1))
			printf("\n");
		else
			printf(" ");
	}
}

/**
 * print_python_float - This prints basic info about Python float objects.
 * @p: This is a  PyObject float object.
 */

void print_python_float(PyObject *p)
{
	char *buffer = NULL;

	PyFloatObject *float_obj = (PyFloatObject *)p;

	fflush(stdout);

	printf("[.] float object info\n");
	if (strcmp(p->ob_type->tp_name, "float") != 0)
	{
		printf("  [ERROR] Invalid Float Object\n");
		return;
	}

	buffer = PyOS_double_to_string(float_obj->ob_fval, 'r', 0,
			Py_DTSF_ADD_DOT_0, NULL);
	printf("  value: %s\n", buffer);
	PyMem_Free(buffer);
}
